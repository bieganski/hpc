...
float cuda_hostalloc_test(int size, bool up) {
    cudaEvent_t start, stop;
    int *a, *dev_a;
    float elapsedTime;
    ...
    cudaError_t err = cudaHostAlloc((void**) &a, size * sizeof(*a), 0); 
...
int main(void) {
    ...
    elapsedTime = cuda_hostalloc_test(SIZE, true);

    printf("Total time for copy up: %3.1f ms\n", elapsedTime);
    printf("\tMB/s during copy up:  %3.1f\n", MB / (elapsedTime / 1000));

    elapsedTime = cuda_hostalloc_test(SIZE, false); 
