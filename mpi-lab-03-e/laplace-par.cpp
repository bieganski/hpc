...
// #de fine ROOT_PROC 0;
// #de fine IMLAST(rank, numProcesses) ((rank) == ( (numProcesses) - 1) );
// #de fine IMROOT(rank) ( (rank) == 0 );
...
static std::tuple<int, double> performAlgorithm(int myRank, int numProcesses, GridFragment *frag, double omega, double epsilon) {
    ...
    do {
        ...
        // MPI_Ibarrier(MPI_COMM_WORLD, &tmp_req);

        if ( myRank != 0 ) {
            // only first process dont need to wait for first row

            MPI_Recv(frag->data[0][0], frag->getNumColorPointsInRow(startRowIncl - 1, 0), MPI_DOUBLE, myRank - 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            MPI_Recv(frag->data[1][0], frag->getNumColorPointsInRow(startRowIncl - 1, 1), MPI_DOUBLE, myRank - 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            // printf("%d: odebrałem dwie wiadomości.\n", myRank);
        }
        //     MPI_Isend(nullptr, 1, MPI_INT, 1, 0, MPI_COMM_WORLD, &tmp_req);

        // MPI_Wait(&tmp_req, MPI_STATUS_IGNORE);
        ...
        if (myRank != numProcesses - 1) {
            // last one don't have to send row to next one

            // for (int i = 0; i < 5; i++) { 
            //     // printf("debug first: %d, %d, dla row = %d\n", frag->getNumColorPointsInRow(startRowIncl, 0), frag->getNumColorPointsInRow(startRowIncl, 1))
            //     // printf("debug last: %d, %d, dla row = %d\n", frag->getNumColorPointsInRow(i, 0), frag->getNumColorPointsInRow(i, 1));
            // }

            int lastIdx = frag->lastRowIdxExcl - frag->firstRowIdxIncl - 1;
            MPI_Send(frag->data[0][lastIdx], frag->getNumColorPointsInRow(lastIdx, 0), MPI_DOUBLE, myRank + 1, 0, MPI_COMM_WORLD);
            MPI_Send(frag->data[1][lastIdx], frag->getNumColorPointsInRow(lastIdx, 1), MPI_DOUBLE, myRank + 1, 0, MPI_COMM_WORLD);
        }


        double globalMaxDiff = 0.0;
        MPI_Allreduce(&maxDiff, &globalMaxDiff, 1, MPI_DOUBLE, MPI_MIN, MPI_COMM_WORLD);
...
int main(int argc, char *argv[]) {
    ...
    auto gridFragment = new GridFragment(numPointsPerDimension, numProcesses, myRank, true);
