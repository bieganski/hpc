...
#include <iterator>
#include <stdio.h>
#define PRINT(begin, end) \
    do { \
        std::copy((begin), (end), std::ostream_iterator<decltype(*begin)>(std::cout, " ")); \
        std::cout << "\n"; \
    } while (0)

// #defi ne PRINT(begin, end)   std::copy((begin), (end), std::ostream_iterator<decltype(*begin)>(std::cout, " "));
...
int main(int argc, char *argv[]) {
    ...
    if (myProcessNo == ROOT_PROCESS) {
        PRINT(myNumbers, myNumbers + NUM_NUMBERS_PER_PROCESS + 1);
    }
