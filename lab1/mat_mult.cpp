...
void task(Matrix first, Matrix second, Matrix result, int size, int iMin, int iMax) {
...
#pragma omp parallel shared(size, first, second, result)
#pragma omp sections
    {
#pragma omp section
        {
            task(first, second, result, size, 0, size / 4 + 1);
        }
#pragma omp section
        {
            task(first, second, result, size, size / 4 + 1, size / 2 + 1);
        }
#pragma omp section
        {
            task(first, second, result, size, size / 2 + 1, size - (size / 4) + 1);
        }
#pragma omp section
        {
            task(first, second, result, size, size - (size / 4) + 1, size);
        }
}
